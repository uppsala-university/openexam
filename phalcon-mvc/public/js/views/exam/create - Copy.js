...
// @Author Ahsan Shahzad [MedfarmDoIT]
		...
		var qsIdJson = {};
		var qJson = {};
		//var topicsToQs = {"topic_id":"1", "topic_questions" : {"1" : "question titel", "2" : "question titel", "3" :  "question titel"}};
...
/*							// send ajax requedt to update question order
							ajax(
								baseURL + 'core/ajax/manager/topic/update', 
...
*/							
							...
							cntr = 1;
							tmp = {};
							tmp = JSON.parse(JSON.stringify(qsJson));
							...
							$('.sortable-qs').each(function(index, element) {
									...
									$(element).find('.q').each(function(i, qNo) {
										if($(qNo).parent().is(':visible')) {
											tmp[cntr] = qsJson[$(qNo).attr('q-no')];
											$(qNo).html("Q"+(cntr++)+":").attr('q-no', (cntr-1));
										}
									});
							});

							qsJson = JSON.parse(JSON.stringify(tmp));
							refreshQs();
...
/*				$(document).on('click', '.prevent', function(event) {
				...
				$(document).on('click' , '.save-exam-settings', function() {
						...
						details = $(settingBox).find('input[name="details[]"]:checked').length+1;
				...
				/*----------------------------------------------*/ 
				/*	Library resource event handler				*/
				/*	@ToDO: Relocation needed?					*/
				/*----------------------------------------------*/
		...
		var saveQuestionToExam 	= function (qId) {
			...
			$("#question-form").find('.q-part').each(function(index, qPart) {
					...
					var qText = jQuery.trim(
						$(CKEDITOR.instances[$(qPart).find('.write_q_ckeditor').attr('id')].getData()).unwrap().html());
			...
			ajax(
				...
				{"exam_id": examId, "topic_id": topicId, "score": totalScore, "name": qIndex, "quest" : JSON.stringify(qJson), "status" : 'active'}, 
				function(qData) {
						...
						$(qCorrectorList).find('li > span').each(function(i, rElem) {
							...
							if(!qId) {							
								...
								ajax(
									baseURL + 'core/ajax/contributor/corrector/' + (qId ? 'update' : 'create'), 
									{"question_id":qData.id, "user":$(rElem).html()}, 
									function(userData) {
										//do nothing for now
									}
								);
						...
						newQ = $('.sortable-q-topic > li:last').find('.sortable-qs > li:last')
								.clone()
									.find('.q')
										.attr('q-no', qIndex)
										.html("Q" + qIndex+":")
										.end()
									.find('.q-txt')
										.html(aPartQtxt.replace(/(<([^>]+)>)/ig,"").substring(0, 75))
										.end();
								...
								if(ansType == 'textbox') {
									...
									jQuery.each( qPartData.ans_area["data"], function( optTitle, optionStatus ) {
										ansTypeHtml += '<div style="padding-top:5px">\
														<input type="checkbox" '+(optionStatus ? 'checked':'')+' disabled> <span>'+optTitle+'</span>\
													   </div>';
									});				
