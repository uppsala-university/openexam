...
class ServiceGenerator
{
        ...
        public function generate($resources = null, $name = "")
        {
                if (!isset($this->_service)) {
                        $this->_service = new ServiceClass($name);
                }
                if (!isset($this->_service->name)) {
                        $this->_service->setName('Generated' . date('Ymd'));
                }
                if (isset($resources)) {
                        $this->_resources = $resources;
                }

                $this->_service->setComment(
                ...
                // 
                $this->_service->addMethod($this->getConstructor());
                $this->_service->addMethod($this->getMethodInitialize());

                if ($this->_options['with-handle']) {
                        $this->_service->addMethod($this->getMethodOpenExam());
                }

                // 
                // Add resource access methods for all requested actions:
                // 
                foreach ($this->_resources as $resource => $actions) {
        ...
        private function addRead($resource, $relations)
        {
                ...
                $this->_service->addUse($restype);
                $this->_service->addUse('OpenExam\Plugins\Security\Model\ObjectAccess');
...
EOL
        ...
        {
                $this->_service->addUse('OpenExam\Library\WebService\Soap\Types\Handle');
...
EOL
        ...
        private function getConstructor()
        {
                ...
                // 
                $this->_service->addMember(new Member('user', 'User', 'The authenticated user.'));
                $this->_service->addUse('OpenExam\Library\Security\User');

                $this->_service->addMember(new Member('core', 'CoreHandler', 'The core handler object.'));
                $this->_service->addUse('OpenExam\Library\Core\Handler\CoreHandler');
...
EOL
        ...
        private function getMethodInitialize()
        {
                ...
                $this->_service->addUse('OpenExam\Library\Security\Exception', 'SecurityException');
                $this->_service->addUse('OpenExam\Library\WebService\Soap\Types\Handle');
