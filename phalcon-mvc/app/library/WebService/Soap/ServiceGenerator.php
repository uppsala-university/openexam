...
class ServiceGenerator
{
                        ...
                        foreach ($actions as $action) {
                                switch ($action) {
                                        case ObjectAccess::CREATE:
                                                $this->addCreate($resource);
                                                break;
                                        case ObjectAccess::READ:
                                                $this->addRead($resource);
                                                break;
         ...
         */
        private function addRead($resource)
        {
                ...
                $method->addCode(
                    <<< EOL
                return \$this->core->action(new $restype(), ObjectAccess::READ);
EOL
                );
...
EOL
         ...
         * @param string $resource The resource name.
         */
        private function addCreate($resource)
        {
                $restype = ModelBase::getRelation($resource);
                $this->service->addUse($restype);
                $this->service->addUse('OpenExam\Plugins\Security\Model\ObjectAccess');
...
EOL
                ...
                $this->addInitilize($method);
                $method->addParam(new Parameter($resource, $restype, "The ${resource} object."));
